...
HttpClient.prototype.save = function(bucket, key, data, options, callback) {
  var Meta = _metaType(options);
  var method = key !== undefined ? 'put' : 'post',
...
HttpClient.prototype._execute = function(meta) {
  ...
  //   var host = meta.host+':'+meta.port
  //     , reqs = request.agent.requests[host] && request.agent.requests[host].length
  //     , socs = request.agent.sockets[host] && request.agent.sockets[host].length;
  ...
  request.on('clientError', function(err){
